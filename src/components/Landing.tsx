...
export default function Landing({ onLogin, isFirebaseConfigured, firebaseConfigError }: LandingProps) {
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      {/* Background decorative blurs — matching the indigo/purple palette */}
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-40 left-1/2 h-[34rem] w-[34rem] -translate-x-1/2 rounded-full bg-indigo-200/40 blur-3xl" />
        <div className="absolute right-[-12rem] top-24 h-96 w-96 rounded-full bg-purple-200/30 blur-3xl" />
        <div className="absolute bottom-[-10rem] left-[-10rem] h-80 w-80 rounded-full bg-indigo-300/20 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl items-center px-4 py-14 sm:px-8 lg:px-12">
        <div className="grid w-full items-center gap-10 lg:grid-cols-[1.08fr_0.92fr]">
          ...
          >
            <div className="mb-8 inline-flex items-center gap-3 rounded-full border border-indigo-200 bg-white/70 px-4 py-2 text-sm font-medium text-indigo-700 backdrop-blur shadow-sm">
          ...
          <motion.section
            ...
            className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl shadow-indigo-100 sm:p-8"
          >
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Welcome to ExamPrep</p>
                <p className="text-lg font-bold text-slate-900">Sign in to continue</p>
              </div>
            </div>
            ...
            <button
              ...
              className="group flex w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-4 text-base font-bold text-white shadow-xl shadow-indigo-200 transition-all hover:scale-[1.01] hover:shadow-indigo-300 disabled:cursor-not-allowed disabled:opacity-55"
            >
            ...
            <div className="mt-6 space-y-3">
              {highlights.map((item) => (
                <div key={item.title} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                  <item.icon className="mt-0.5 h-4.5 w-4.5 flex-shrink-0 text-indigo-500" />
                  <div>
